        ...
        try:
            ...
            for item in parsed:
                if isinstance(item, dict) and 'keyword' in item and 'volume' in item and 'difficulty' in item:
                    # ASTRA AI Quality Improvement:
                    # Wrapped type casting inside the array parsing loop in a try/except block.
                    # This ensures that a single malformed item (e.g., volume="10K") doesn't throw a ValueError
                    # that discards the entire array of valid items.
                    try:
